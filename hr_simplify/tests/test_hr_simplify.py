...
class TestHrSimplify(SavepointCase):
    ...
    def create_contract(
        ...
        self.contract_data["date_start"] = (
            datetime.now().date() if start_date is None else start_date
        )
    ...
    def test_compute_employees(self):
        self.create_contract(
            ...
            start_date=(datetime.now() - timedelta(days=(5 * 30))).date(),
            end_date=(datetime.now() - timedelta(days=(2 * 30))).date(),
        )
        self.create_contract(
            name="Tim Contract",
            employee=self.emp_tim,
            job=self.job_rnd_technician,
            start_date=(datetime.now() - timedelta(days=(6 * 30))).date(),
            end_date=(datetime.now() - timedelta(days=(2 * 30))).date(),
        )
        self.create_contract(
            name="Ed Contract",
            ...
            job=self.job_rnd_technician,
            start_date=(datetime.now() - timedelta(days=(4 * 30))).date(),
